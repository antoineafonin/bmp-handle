...
int main(int argc, char **argv) {
            ...
            case 'c': {
                ...
                for (size_t i = 0; i < opt_size; ++i) {
                    if (optarg[i] == ',' || i == opt_size - 1) {
                        if (i != opt_size - 1) optarg[i] = 0;
                        crop_params[param_index++] =
                            atoi(optarg + num_start_pos);
                        num_start_pos = i + 1;
                    }
                }
    ...
    if (file_input != NULL) input = fopen(file_input, "r");

    if (file_output != NULL) output = fopen(file_output, "w+");
    ...
    if (crop_f) {
        image_tmp = crop(image, crop_params[0], crop_params[1], crop_params[2],
                         crop_params[3]);
        free_bmp_image(image);
        image = image_tmp;
    }
